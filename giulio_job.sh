...
#PBS -q gpu
#PBS -l select=4:ncpus=48:ngpus=4:mpiprocs=4
#PBS -k oe
#PBS -j oe
...
conda activate giuder >/dev/null 2>&1
...
for ((i = 0; i < NODES; i++)); do
    ...
    PYTHON_EXEC="accelerate launch $ACCELERATE_ENVS train_accelerate.py --quantization 8bit --max_length 512 --version 8"
